#include "DecryptAES.h"

const uint8_t DecryptAES::INV_S_BOX[] = {  };

DecryptAES::DecryptAES() {
	initAes(128, "");
}

DecryptAES::DecryptAES(const std::string &txt) {
	initAes(128, txt);
}

DecryptAES::DecryptAES(const unsigned int &ver) {
	initAes(ver, "");
}

DecryptAES::DecryptAES(const unsigned int &ver, const std::string &txt) {
	initAes(ver, txt);
}

void DecryptAES::invSubBytes() {
	uint8_t curElem;
	for (uint8_t elem = 0; elem < 4; elem++) {
		for (uint8_t elem2 = 0; elem2 < 4; elem2++) {
			curElem = state[elem][elem2];
			state[elem][elem2] = INV_S_BOX[uint8_t(
					(0xf0 & curElem) | (0x0f & curElem))];
		}
	}
}

void DecryptAES::invShiftRows() {
	std::vector<uint8_t> prom(4);
	for (uint8_t r = 1; r < 4; r++) {
		for (uint8_t c = 0; c < Nb; c++) {
			prom[(c + r) & 3] = state[r][c];
		}
		state[r] = prom;
	}
}

void DecryptAES::invMixColumns() {
	uint8_t a, b, e, d;
	for (uint8_t c = 0; c < Nb; c++) {
		a = g_mul(0x0e, state[0][c]) ^ g_mul(0x0b, state[1][c])
				^ g_mul(0x0d, state[2][c]) ^ g_mul(0x09, state[3][c]);
		b = g_mul(0x09, state[0][c]) ^ g_mul(0x0e, state[1][c])
				^ g_mul(0x0b, state[2][c]) ^ g_mul(0x0d, state[3][c]);
		e = g_mul(0x0d, state[0][c]) ^ g_mul(0x09, state[1][c])
				^ g_mul(0x0e, state[2][c]) ^ g_mul(0x0b, state[3][c]);
		d = g_mul(0x0b, state[0][c]) ^ g_mul(0x0d, state[1][c])
				^ g_mul(0x09, state[2][c]) ^ g_mul(0x0e, state[3][c]);
		state[0][c] = a;
		state[1][c] = b;
		state[2][c] = e;
		state[3][c] = d;
	}
}

void DecryptAES::decryptAes() {
	addRoundKey(Nr);
	for (uint8_t round = Nr - 1; round > 0; round--) {
		invShiftRows();
		invSubBytes();
		addRoundKey(round);
		invMixColumns();
	}
	invShiftRows();
	invSubBytes();
	addRoundKey(0);
}

void DecryptAES::decrypt(std::string &resultEncrypt,
		const std::vector<uint8_t> &k) {
	keyExpansion(k);
	if (resultEncrypt.capacity() < text.size())
		resultEncrypt.reserve(this->text.size());
	int index = 0, jndex = 0;
	for (auto iter = this->text.begin(); iter != this->text.end();
			iter++, index++) {
		state[index][jndex] = (*iter);
		if (index == 3) {
			index = -1;
			if (jndex == 3) {
				decryptAes();
				for (int i = 0; i < 4; i++)
					for (int j = 0; j < 4; j++)
						resultEncrypt += state[j][i];
				jndex = 0;
			}
			else
				jndex++;
		}
	}
}

std::string DecryptAES::decrypt(const std::vector<uint8_t> &key) {
	std::string result = "";
	decrypt(result, key);
	return result;
}
